...
BOOL CTestServerDlg::OnInitDialog()
{
	...
	AddEventString("클라이언트 접속을 허락합니다...");
...
void CTestServerDlg::AcceptProcess(SOCKET parm_h_socket) {
	if (MAX_CLIENT_COUNT > m_client_count) {
		...
		//클라이언트 주소를 알고 싶을때 
		CString ip_address;
		wcscpy(m_client_ip[m_client_count], (const wchar_t*)&ip_address);
		ip_address = inet_ntoa(client_addr.sin_addr);

		AddEventString("새로운 클라이언트가 접속을 했습니다.:" + ip_address);
...
void CTestServerDlg::SendFrameData(SOCKET param_h_socket, unsigned char param_id, const void* parm_p_data, int param_size)
{
...
LRESULT CTestServerDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (27001 == message) {
		...
		if (WSAGETSELECTEVENT(lParam) == FD_READ) {
					...
					if (network_message_id == 1) {
						...
						for (int i = 0; i < m_client_count; i++) {
							if (h_socket == mh_client_list[i]) {
								str.Format("%s : %s", m_client_ip[i], p_body_data);
								break;
							}
						}
			...
			ClientCloseProcess(h_socket, 0); // FD_CLOSE
			AddEventString("클라이언트가 접속을 해제했습니다.");
			}
	}
